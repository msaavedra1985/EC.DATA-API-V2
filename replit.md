...
- **Authentication:** Comprehensive JWT-based system with access/refresh tokens, token rotation, theft detection, and role-based access control (RBAC). Refresh tokens are database-persisted and SHA-256 hashed. JWT includes standard claims (`iss`, `aud`, `sub`, `iat`, `exp`, `jti`, `orgId`, `sessionVersion`, `tokenType`).
