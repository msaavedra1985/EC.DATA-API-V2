...
Los query params usan **snake_case**. El DTO los transforma internamente a camelCase para los servicios:

| Query param (HTTP) | Campo interno (servicio) |
|--------------------|--------------------------|
| `parent_id` | `parentId` |
| `node_type` | `nodeType` |
| `is_active` | `isActive` |
...
## Resolución de `organizationId` en escrituras (POST, batch-create)

Los endpoints de escritura (`POST /nodes`, `POST /nodes/batch-create`) resuelven la organización mediante `resolveRequestOrganizationId()`:
...
> Los endpoints de lectura (`GET /nodes`, `GET /tree`, etc.) usan el `organizationContext` establecido por `enforceActiveOrganization`. Si el usuario está en "God View" (`showAll=true`), no filtra por organización.
...
| `node_type` | string | — | Filtrar: `folder`, `site` o `channel` |
...
|-------|------|---------|-------------|
| `node_type` | string | — | Filtrar: `folder`, `site` o `channel` |
| `max_depth` | integer | sin límite | Profundidad máxima relativa (1=hijos, 2=hijos+nietos, etc.) |
...
| Param | Tipo | Default | Descripción |
|-------|------|---------|-------------|
| `category_id` | integer | **requerido** | ID de la categoría de asset |
| `organizationId` | string | org activa | Public code de la organización |
| `include_subcategories` | boolean | `true` | Incluir subcategorías del tag |

### Ejemplo

**Request:**
```
GET /api/v1/resource-hierarchy/tree/filter?category_id=42
```
